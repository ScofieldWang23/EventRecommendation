...
(function() {
  ...
  function login() {
    ...
    password = md5(username + md5(password)); // 两次加密
  ...
  function ajax(method, url, data, successCallback, errorCallback) {
    ...
    xhr.open(method, url, true); // true: asynchronous，可以不写
  ...
  function changeFavoriteItem(item_id) {
    ...
    // request parameters
    var url = './history';
    var req = JSON.stringify({ // serialize
      user_id: user_id,
      favorite: [item_id]
    });
    ...
    ajax(method, url, req,
      // successful callback
      function(res) {
        var result = JSON.parse(res); // parse JSONString, convert to object
